	...
	while(check_state(J))
	{
		...
		while (Cons_Cand_List(J, S.v_Smachines.at(M_num - 1).v_Mbatches.at(v_B_size).get_rs()).size() != 0)
		{
			...
			if (S.v_Smachines.at(M_num - 1).v_Mbatches.at(v_B_size).get_rs() == S.v_Smachines.at(M_num - 1).get_s())
			{
				...
				{
					j->selection_feature = (static_cast<double>(j->get_r()) + j->get_p()/ S.v_Smachines.at(M_num - 1).get_v()) * abs(static_cast<double>(j->get_r()) + j->get_p()/ S.v_Smachines.at(M_num - 1).get_v() - j->get_d());
				}
